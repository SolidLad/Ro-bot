...
public class Mute implements Command {
    ...
    public void run(MessageReceivedEvent event, String[] args) {GuildManager gm = new GuildManager(event.getGuild());
        new Thread(()-> {
            ...
            if ((utils.GuildManager.getBotPerms(event.getGuild()).contains(Permission.VOICE_MUTE_OTHERS)&&authorPerms.contains(Permission.VOICE_MUTE_OTHERS))||utils.GuildManager.getBotPerms(event.getGuild()).contains(Permission.ADMINISTRATOR)&&authorPerms.contains(Permission.ADMINISTRATOR)) {
                gm.mute(target);
            }
        }).run();
